...
async def create_thumb_grid(frames_folder: Path, output_path: Path, grid_size: tuple = (4, 4)) -> Path:
    """
    ...
    try:
        ...
        if len(frame_files) < grid_size[0] * grid_size[1]:
            print(f"Warning: Not enough frames for grid. Found {len(frame_files)}, need {grid_size[0] * grid_size[1]}")
...
async def process_video_thumb(video_path: Path, thumb_path: Path, frames: int = 16, grid: tuple = (4, 4)) -> Path:
    """
