...
class DownloadManager:
    ...
    async def _download_single_media(self, task: DownloadTask, source_message: Message) -> Optional[Path]:
        ...
        try:
            if not source_message.media:
                if source_message.text:
                    ...
                    await task.status_message.edit_text("✅ Text message processed (no media)")
                else:
                    await task.status_message.edit_text("❌ No media or text found in the message")
                return None
            ...
            # Update status
            await task.status_message.edit_text("✅ Download completed. Queued for upload...")
    ...
    async def _download_media_group(self, task: DownloadTask, source_message: Message) -> List[Path]:
        ...
        try:
            ...
            await task.status_message.edit_text(f"❌ Media group download failed: {str(e)}")
            raise
