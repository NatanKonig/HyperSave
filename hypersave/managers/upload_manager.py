        ...
        async with self.upload_semaphore:
            try:
                ...
                if (
                    ...
                    or task.last_progress_text != "📤 Upload started..."
                ):
                    try:
                ...
                if (
                    not hasattr(task, "last_progress_text")
                    or task.last_progress_text != "✅ Upload completed!"
                ):
                    try:
    ...
    async def _upload_media_group(self, task: UploadTask):
        ...
        try:
            ...
            if media_list:
                ...
                final_status = (
                    f"✅ Grupo de mídia enviado com sucesso! ({len(media_list)} itens)"
                )
                if (
                    not hasattr(task, "last_progress_text")
                    or task.last_progress_text != final_status
                ):
                    try:
                ...
                await task.status_message.edit_text(
                    "❌ Nenhum arquivo de mídia válido encontrado no grupo"
                )

        except Exception as e:
            await task.status_message.edit_text(
                f"❌ Erro ao processar grupo de mídia: {str(e)}"
            )
            raise
